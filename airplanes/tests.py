...
class AirplaneModelTest(TestCase):
    def setUp(self):
        ...
        self.airplane = Airplane.objects.create(
            user_defined_airplane_id=1, passenger_count=100, created_by=self.user
        )

    def test_fuel_tank_capacity(self):
        expected_capacity = 200 * self.airplane.user_defined_airplane_id
        self.assertEqual(self.airplane.fuel_tank_capacity, expected_capacity)

    def test_fuel_consumption_per_minute(self):
        expected_consumption = (
            log(self.airplane.user_defined_airplane_id) * 0.80
            + self.airplane.passenger_count * 0.002
        )
        self.assertEqual(
    ...
    def test_str(self):
        expected_str = f"Airplane {self.airplane.user_defined_airplane_id} with {self.airplane.passenger_count} passengers"
